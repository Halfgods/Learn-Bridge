    ...
    <div className="p-6 lg:p-10 max-w-5xl mx-auto space-y-6">
      ...
      <header className="clay-lg bg-card p-6 flex items-center gap-5">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">{chapters.length} chapters • Adaptive learning path</p>
        </div>
      </header>
      ...
      <div className="clay-lg bg-card p-4 space-y-2">
        {isLoading && <div className="p-8 flex justify-center"><Loader2 className="w-8 h-8 animate-spin text-primary"/></div>}
        {error && <div className="p-8 text-center text-destructive font-bold">Failed to load chapters.</div>}
        {!isLoading && !error && chapters.length === 0 && <div className="p-8 text-center text-muted-foreground font-bold">No chapters found for this subject.</div>}
        ...
        {chapters.filter((ch: any) => {
          ...
          return (
            <div key={ch.chapterName} className="rounded-2xl">
              ...
              >
                <div className={cn("h-9 w-9 rounded-xl flex items-center justify-center clay-sm",
      ...
      <div className="grid sm:grid-cols-4 gap-3">
        {[
          { icon: FileText, label: "PDFs", value: "32", color: "gradient-yellow text-amber-900" },
          { icon: Network, label: "Concept maps", value: "12", color: "gradient-cyan text-white" },
          { icon: Youtube, label: "Videos", value: "48", color: "gradient-peach text-orange-900" },
          { icon: BarChart3, label: "Insights", value: "Live", color: "gradient-mint text-emerald-900" },
        ].map((c) => (
          <div key={c.label} className="clay-sm bg-card p-4">
            <div className={cn("h-10 w-10 rounded-xl flex items-center justify-center mb-2", c.color)}>
